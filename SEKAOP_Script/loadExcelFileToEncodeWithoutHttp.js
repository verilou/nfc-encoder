    ...
    file.eachSheet(async (worksheet) => {
      ...
      for (let index = 0; index < rowTotal; index++) {
        ...
        if (row.values.length > 0 && !row.getCell("encoded").value) {
          const path = row.getCell("path").value;
          try {
            ...
            throw new Error(
              "Something went wrong try to launch the script again"
            );
          }
          row.getCell("encoded").value = true;
          row.commit();
          row.getCell("cardUID").value = card.uid;
          row.commit();
          await file.xlsx.writeFile(excelPath.trim());
          return;
        }
...
const writeCard = async (string, reader) => {
  const textRecord = ndef.Utils.createTextRecord(string.trim());
  console.log(string);
  ...
  await reader.write(43, Buffer.from([0x43, 0x41, 0x52, 0x44]));
